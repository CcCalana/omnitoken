...
func TestRequireVirtualKey(t *testing.T) {
	...
	handler := RequireVirtualKey(fakeStore{record: record}, func(w http.ResponseWriter) {
...
func TestRequireVirtualKeyRejectsMissingHeader(t *testing.T) {
	handler := RequireVirtualKey(fakeStore{}, func(w http.ResponseWriter) {
		w.WriteHeader(http.StatusUnauthorized)
...
func TestRequireVirtualKeyReturnsServerErrorForStoreFailure(t *testing.T) {
	...
	handler := RequireVirtualKey(fakeStore{err: context.Canceled}, func(w http.ResponseWriter) {
		w.WriteHeader(http.StatusUnauthorized)
	})(http.HandlerFunc(func(http.ResponseWriter, *http.Request) {
		t.Fatal("next should not be called")
	}))
