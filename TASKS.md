...
## T-007 SSE 反向代理 + 方舟 OpenAI-compat 转发 [phase:1] [owner:codex] [status:review]
...
**Result**: `34a5b6a` — implemented the Demo-Ready Ark OpenAI-compatible chat proxy with JSON request rewriting, non-stream passthrough, SSE chunk flushing, safe upstream header filtering, `//go:build e2e` integration coverage, and local-dev curl examples. Self-test: `go test ./...`; `go vet ./...`; `go test -cover ./internal/proxy` (88.4%); `go test -tags=e2e ./internal/proxy` with `OMNITOKEN_ARK_API_KEY` empty so it compiled and skipped without hitting real Ark; `gofmt -l cmd internal`; gateway Docker build passed.
