...
| 6 | T-044 路由规则联动 | virtual_models CRUD API + `omnitoken adopt --ensure-model` + admin UI 编辑 | 2d | T-040 | ✅ `8fb054e` |
| 7 | T-046 一键 onboard CLI 收口 | `omnitoken adopt <agent>` 交互式 prompts + status + dry-run + 输出 polish | 1d | T-044 + T-045 | ✅ `00c9f4a` |

**Phase 3-A: 7/7 done ✅ — Agent 适配 Epic 圆满收官**
...
## T-044 路由规则联动 — virtual_models CRUD + Agent 配置同步 [phase:3-A] [owner:codex] [status:done] [started:2026-05-30 11:27 CST]
...
## T-046 一键 onboard CLI 收口 [phase:3-A] [owner:codex] [status:done] [started:2026-05-30 13:41 CST]

**目标**: 把 `omnitoken-adopt` 从"传齐 8 个 flag 才能跑"升级为"一键交互式 onboard"——缺 flag 时交互式提示、`status` 命令查看当前配置、`--dry-run` 预览不写盘、adopt/restore 完成后输出清晰摘要。**Phase 3-A 最后一块拼图**，完成后 Agent 适配 Epic 收官。

**背景**: T-041~T-044 把 adopt 的功能做全了（三种 agent 配置写入 + virtual model ensure），但 CLI 体验仍是"传 flag → 静默成功或报错"。新用户不知道要传哪些 flag、不知道当前 agent 配了没、配错了不知道。T-046 补齐交互层，让 `omnitoken-adopt` 从 dev tool 变成 user-facing tool。

**涉及**:
- `cmd/omnitoken-adopt/main.go` — 加交互式 prompts + `status` 子命令 + `--dry-run` + 输出 polish
- `cmd/omnitoken-adopt/main_test.go` — 状态查询 / dry-run / 交互式输入测试

**接受标准**:
- [ ] **交互式 prompts**：当必需 flag（`--gateway-url` / `--token`）缺失时，从 stdin 提示输入。可选 flag（`--model` / `--admin-url` / `--real-model` / `--provider`）缺失时提示并允许回车跳过（用默认值或跳过 ensure）。示例交互：
  ```
  $ omnitoken-adopt claude-code
  Gateway URL: http://localhost:8080
  Virtual key token: sk-xxx
  Model [chat-balanced]:
  Admin URL (skip to disable auto-ensure) [http://localhost:8081]:
  Real model [deepseek-v4-flash]:
  Provider [deepseek]:
  ✓ Virtual model "chat-balanced" confirmed
  ✓ Claude Code configured → ~/.claude/settings.json
  ```
- [ ] **`status` 子命令**：`omnitoken-adopt status [claude-code|codex|opencode]` 读取 agent 现有配置文件，打印当前设置摘要（gateway URL / model / token prefix）。如果未配置，打印"未配置"。不写任何文件。实现方式：调用既有 `AgentConfig` 接口的某种 read/status 方法，或在 CLI 内直接解析 known config 路径
- [ ] **`status` 不指定 agent 时列出全部**：`omnitoken-adopt status`（无参数）列出三种 agent 各自的状态（已配置 / 未配置），一行一个
- [ ] **`--dry-run` flag**：`omnitoken-adopt adopt claude-code --dry-run ...` 执行全部 validation + ensure model 检查，但不写任何文件。输出"Would write settings to ~/.claude/settings.json"等信息。和 `status` 的区别：`status` 看现有配置；`dry-run` 预览将要做的改动
- [ ] **输出 polish**：adopt 成功后打印 3-5 行摘要（agent 类型 / config 路径 / gateway URL / model / backup 路径）。restore 成功后打印"已恢复原始配置" + 清理的文件列表
- [ ] **restore 前确认**：`omnitoken-adopt restore claude-code` 在覆盖前提示"将恢复 ~/.claude/settings.json 到 adoption 前的备份。继续？[y/N]"。tty 环境才提示；非 tty（CI/脚本）默认 yes（不阻塞自动化）
- [ ] **错误消息 polish**：常见错误给出 actionable 建议——
  - "gateway unreachable" → "请检查 --gateway-url 是否正确，gateway 是否在运行"
  - "admin API 401" → "请确认 --token 是有效的 admin 角色虚拟 key"
  - "virtual model conflict" → "virtual model 'X' 已存在但 provider/real_model 不同。请用 admin web 编辑或删除后重试"
- [ ] 既有 flag 全部保留，交互式 prompts 仅在 flag 缺失且 stdin 是 tty 时触发（非 tty 时缺 flag 直接报错，不阻塞 CI）
- [ ] `go vet ./...` + `go test -race ./...`（Docker 内）全绿；`cmd/omnitoken-adopt` 测试覆盖率不退化

**不在范围**:
- ❌ 多 agent 批量 adopt（一次配多个 agent）—— v1 单 agent 单次
- ❌ `omnitoken-adopt update`（修改现有配置）—— restore + adopt 重新配等同
- ❌ `omnitoken-adopt remove`（清理由 restore 覆盖）
- ❌ GUI / TUI —— 保持纯 CLI stdin prompt
- ❌ agent config schema validation（如校验 gateway URL 可达性）—— 留给后续 `omnitoken doctor`
- ❌ 修改 `internal/agent_adapter/` 接口

**Codex propose 前置**: **跳过**。范围由 TASKS 直接锁定。实施时如遇未声明偏差停并向 Claude 报告。

**依赖**: T-041~T-044 全部 ✅（adopt 全功能 + virtual model ensure）；T-045 ✅（gateway `/v1/messages` 可用，status 命令可验证端点可达性）

**Hints**:
- **交互式 prompts**：用 `bufio.NewScanner(os.Stdin)` + `fmt.Printf`。检测 tty：`term.IsTerminal(int(os.Stdin.Fd()))`（`golang.org/x/term`）或简单检查 `os.Stdin` 的 `Stat()` mode。`golang.org/x/term` 已是间接依赖（通过其他包），可直接 import
- **`status` 命令**：三种 agent 的配置路径已知（`~/.claude/settings.json` / `~/.codex/config.toml` / `~/.config/opencode/opencode.json`）。可直接解析 JSON/TOML 读关键字段，或复用 `agent_adapter` 包的现有 read 逻辑。如果加新方法到 `AgentConfig` interface 太重，直接在 CLI 内做轻量解析
- **restore 确认**：用 `fmt.Printf("将恢复 %s。继续？[y/N]: ", path)` + `bufio.Scanner` 读一行。非 tty（CI）跳过确认直接 yes
- **dry-run**：本质是把 `Write()` 调用替换为 `fmt.Printf("Would write to %s\n", path)` + 打印将要写入的配置摘要。可以和正常路径共享 flag parsing + validation，差异仅在最后一步
- **输出格式**：成功用 `✓` 前缀 + 英文/中文混合（与现有 adopt 输出风格一致）。错误用 `✗` 前缀
- **commit 策略**：建议单 commit——改动集中在 `cmd/omnitoken-adopt/`，体积可控

**禁动**:
- `internal/agent_adapter/` 既有接口（不强制加 `Status()` 或 `DryRun()` 方法；如果 Codex 判断值得加，作为建议写到 PR 描述，由 reviewer 决定）
- 既有 adopt/restore 命令行参数语义（零 breaking change）
- 既有测试的行为（可新增 case，不改现有断言）

Result: `00c9f4a` — prompts/status/dry-run/restore-confirm/error polish landed; all green, no deviation.
...
**背景**: Claude 已产出 4 个部署文件（`docs/operations/server-deployment.md`、`deploy/nginx/nginx.conf`、`deploy/Dockerfile.nginx`、`deploy/docker-compose.server.yml`），但尚未经过验证。Codex 需要在部署前做一轮 correctness check + 补上自动化验证脚本。

**涉及**:
- `deploy/docker-compose.server.yml` — 语法检查 + 本地 `docker compose config` 验证
- `deploy/nginx/nginx.conf` — `nginx -t` 语法检查
- `deploy/Dockerfile.nginx` — `docker build` 验证
- `scripts/deploy-check.sh` (新增) — 服务器端部署前环境检查脚本
- `scripts/deploy-smoke.sh` (新增) — 部署后 smoketest 脚本

**接受标准**:
- [ ] `docker compose -f deploy/docker-compose.server.yml config` 无报错（在项目根目录跑）
- [ ] `docker compose -f deploy/docker-compose.server.yml build nginx` 构建成功
- [ ] nginx 配置语法验证通过：`docker run --rm -v $(pwd)/deploy/nginx/nginx.conf:/etc/nginx/nginx.conf:ro nginx:1.27-alpine nginx -t`（或等价的 compose build 后容器内检查）
- [ ] **`scripts/deploy-check.sh`** 服务器预检脚本（可独立运行，不依赖 docker-compose），检查：
  - Docker 版本 ≥ 24、docker-compose 可用
  - 端口 443、80 未被占用
  - `.env` 文件存在且 `OMNITOKEN_DEEPSEEK_KEYS_1/2/3` 非空（至少 1 把 key）
  - `.omnitoken-master-key` 文件存在且长度 ≥ 64 字符（hex 编码后 32 字节）
  - `deploy/ssl/server.crt` 和 `server.key` 存在
  - 内存 ≥ 4 GB（`free -m` 检查）
  - 磁盘可用空间 ≥ 10 GB
- [ ] **`scripts/deploy-smoke.sh`** 部署后 smoketest 脚本（在服务器上跑，需传 `<SERVER-IP>` 和 `<VIRTUAL-KEY>` 参数），验证：
  - `GET /healthz` 返回 200
  - `POST /v1/messages`（Anthropic 格式 + `x-api-key` header）返回 200 + `"type":"message"`
  - `POST /v1/chat/completions`（OpenAI 格式 + `Authorization: Bearer` header）返回 200 + `"choices"` 字段
  - 两条都用 `--cacert deploy/ssl/server.crt` 信任自签证书
- [ ] 通读 `docs/operations/server-deployment.md` runbook，发现任何缺失/矛盾/不清之处写进未声明偏差（不直接改文件——Claude 维护 runbook）
- [ ] **Docker-only**：所有可本地跑的验证在 docker 内完成

**不在范围**:
- ❌ Codex 不实际 SSH 到服务器执行部署（用户手动操作）
- ❌ 不修改 `deploy/docker-compose.yml`（本地开发用）、不修改 `deploy/Dockerfile.gateway` / `Dockerfile.admin` / `Dockerfile.migrate`
- ❌ 不创建新的 Go 代码或修改 `internal/` / `cmd/`
- ❌ 不修改 nginx.conf 或 docker-compose.server.yml（如果有问题写进未声明偏差，Claude 修）

**Codex propose 前置**: **跳过**。范围由 TASKS 直接锁定。

**依赖**: T-045 ✅（`/v1/messages` 端点）；T-FIX-AUTH-XAPIKEY ✅（`x-api-key` fallback）

**Hints**:
- **docker compose config 验证**: 需在项目根目录跑，因为 Dockerfile 的 `context: ..` 会找上层。`docker compose -f deploy/docker-compose.server.yml config` 如果报 `services.xxx.build.context` 路径错误，可能需要先 `cd deploy && docker compose -f docker-compose.server.yml config`（取决于 context 路径的解析方式。task 内验证并报告）
- **nginx 配置检查**: 可以用 `docker run --rm -v $(pwd)/deploy/nginx/nginx.conf:/etc/nginx/nginx.conf:ro nginx:1.27-alpine nginx -t`——这验证语法，不验证 upstream 可达性
- **smoketest 脚本**: 用纯 bash + curl + jq（服务器上先 `apt install -y jq`）。每条 curl 后检查 HTTP status 和 JSON 字段。`set -euo pipefail` 确保任何失败立即退出
- **preflight 脚本**: 同样纯 bash。每个检查项打印 `✓` 或 `✗`。全部通过才 exit 0，任何失败 exit 1 + 打印修复建议
- **runbook review**: 重点检查 runbook 中的命令是否可直接复制粘贴执行（路径是否正确、env var 名称是否正确、有没有遗漏的步骤）

**禁动**:
- `deploy/docker-compose.yml`（本地开发）
...
**背景**: 当前 e2e 测试（`anthropic_e2e_test.go`）用 `auth.WithSubject` 直接注入 Subject 绕过 auth middleware。T-045 handler 单测用 mock upstream 绕过真实网络。`docker compose up` 起全栈后没有任何自动化测试验过"Claude Code 发 `x-api-key` → gateway auth → proxy → 上游 → Anthropic 响应"。

**涉及**:
- `cmd/gateway/agent_smoke_test.go` (新增，build tag `e2e`) — 真上游 + 走完整 mux 的 agent 调用测试
- `internal/proxy/agent_integration_test.go` (新增，无 build tag) — httptest mock 上游 + 走完整 middleware 栈的 auth + 转换测试
- `testdata/golden/ark/anthropic_nonstream_default.json` — 已有的 Claude Code 请求 fixture（`_meta.request` 字段）

**接受标准**:

**A. 集成测试（无 e2e tag，不依赖真上游，CI 必跑）**:
- [ ] **Claude Code 模拟 + mock 上游**：启动 `httptest.Server` mock 上游（返固定 OpenAI JSON chunk），构造 gateway mux（真 middleware 栈：`protectGatewayRoute → enforceMonthlyBudget → resolveVirtualModel → anthropic.MessagesHandler → usage.Middleware → proxy`，proxy 指向 mock），`POST /v1/messages` + `x-api-key` header + Anthropic body → 断言 200 + `type: "message"` + `usage.input_tokens > 0`
- [ ] **Codex 模拟 + mock 上游**：同上但 `POST /v1/chat/completions` + `Authorization: Bearer` header + OpenAI body → 断言 200 + `choices[0].message.content` + `usage`
- [ ] **错误 x-api-key → Anthropic 401**：`POST /v1/messages` + 无效 `x-api-key` → 断言 401 + `{"type":"error","error":{"type":"authentication_error"}}`（**不是** OpenAI 格式的 `{"error":{...}}`）
- [ ] **错误 Bearer token → OpenAI 401**：`POST /v1/chat/completions` + 无效 `Authorization: Bearer` → 断言 401 + `{"error":{"type":"authentication_error","code":"invalid_api_key"}}`
- [ ] **缺失所有 auth header → 401**：两个 endpoint 都 401，不管路径
- [ ] `go test -race ./...`（不含 e2e tag）全绿

**B. e2e 测试（build tag `e2e`，需真上游，本地/CI 手动触发）**:
- [ ] **Claude Code 非流式 e2e**：`POST /v1/messages` + `x-api-key` + Anthropic body（从 `testdata/golden/ark/anthropic_nonstream_default.json` 的 `_meta.request` 取）→ 断言 200 + `type:"message"` + `usage.input_tokens + output_tokens > 0`
- [ ] **Claude Code 流式 e2e**：`POST /v1/messages` + `x-api-key` + Anthropic body（`stream: true`）→ 断言 SSE 事件流含 `message_start` / `content_block_delta` / `message_delta` / `message_stop`
- [ ] **Codex e2e**：`POST /v1/chat/completions` + `Authorization: Bearer` + OpenAI body → 断言 200 + `choices[0].message` + `usage`
- [ ] **流式 Codex e2e**：同上 `stream: true` → 断言 SSE 事件 + `[DONE]`
- [ ] 上述全部测试同时验证 `usage_events` 表正确写入了 `model_routed` + `upstream_credential_id`（至少查一条 `SELECT` 确认非空）。如果测试环境没配 PG 直连权限，退而求其次：断言 response body 含完整 usage 字段
- [ ] e2e 测试有 `MAX_REQUESTS` 硬上限保护（默认 10），避免成本失控
- [ ] 不需要 `OMNITOKEN_ARK_API_KEY` 时 `t.Skip` 并说明所需 env vars

**不在范围**:
- ❌ 真 Claude Code / Codex / OpenCode 进程调起 —— 纯 HTTP client 模拟
- ❌ `omnitoken-adopt` CLI 端到端（写配置 → 实际 agent 调用）
- ❌ 负载 / 并发测试（已有 T-CONC-CHECK / T-CONC-RERUN）
- ❌ OpenCode 独立测试（协议与 Codex 相同，Codex 路径覆盖等效）
- ❌ 修改 `internal/` 或 `cmd/gateway` 核心代码（只加测试文件）

**Codex propose 前置**: **跳过**。范围由 TASKS 直接锁定。

**依赖**: T-045 ✅（`/v1/messages` 端点 + Anthropic 转换）；T-FIX-AUTH-XAPIKEY (`9958f47`) ✅（`x-api-key` header fallback）；docker-compose 环境（pg/redis/nats + seed）

**Hints**:
- **集成测试 mock 上游**：用 `httptest.NewServer(http.HandlerFunc(...))` 起 mock，返固定 JSON（`{"id":"1","choices":[{"message":{"content":"pong"},"finish_reason":"stop"}],"usage":{...}}`）。流式 mock 返 `data: {...}\n\n` + `data: [DONE]\n\n`。proxy 的 `BaseURL` 指向 mock server URL
- **集成测试构造 mux**：复用 `newMux`（`cmd/gateway/main.go`），传 `auth.NewPostgresStore(db)` + `quota.NewPostgresStore(db)` + `router.NewPostgresStore(db)` + `credentials.NewSelector`。需要 `sql.Open` 连接测试 PG（docker compose 内）。如果 PG 太重，用 `newMux` 的简化版：auth store 用 `auth.NewStaticStore(map[string]VirtualKeyRecord{...})`（如果存在，否则写一个测试用 stub）
- **virtual key 预置**：集成测试需要至少一个 valid virtual key。如果 `auth.NewStaticStore` 不存在，建议加一个 `auth.NewFakeStore` 或 `auth.NewMemoryStore`（测试专用），不要在生产代码里加
- **budget checker 绕过**：`enforceMonthlyBudget` 调用 `quota.BudgetChecker`。集成测试可用 stub（`quota.NoOpBudgetChecker` 或返回 `true` 的 mock）。如果不存在，在测试文件内写简单 stub
- **golden fixture**：`testdata/golden/ark/anthropic_nonstream_default.json` 的 `_meta.request` 字段是现成的 Anthropic-format 请求体
- **commit 策略**: 建议两个 commit —— C1 集成测试（无 e2e tag，CI 必跑）+ C2 e2e 测试（build tag `e2e`，手动触发）
